...
export function SectionTitle({ eyebrow, title, description, action, inverted = false }: SectionTitleProps) {
  const titleClass = inverted ? 'text-cream' : 'text-coffee dark:text-cream';
  const descriptionClass = inverted ? 'text-cream/82' : 'text-coffee/74 dark:text-cream/80';
  ...
  return (
    ...
    >
      <div className="max-w-2xl">
        {eyebrow && <p className="mb-3 text-sm font-bold uppercase tracking-[0.18em] text-caramel">{eyebrow}</p>}
